...
if cfg["show_ma"]:
    for period, color in [(20, GOLD), (50, OR2), (200, CREAM)]:
        fig.add_trace(go.Scatter(x=feat.index, y=feat[f"MA_{period}"],
...
layout["xaxis4"] = dict(rangeslider=dict(visible=False)) if cfg["show_vol"] else {}
...
with c1:
    ...
    glass_card(f"""
      ...
      <div class="stat-number" style="color:{'#D95F4B' if rsi>70 else ('#52B788' if rsi<30 else '#D4621A')}">{rsi:.1f}</div>
      <div style="margin-top:0.5rem">{pill('Overbought' if rsi>70 else ('Oversold' if rsi<30 else 'Neutral'), rsi_kind)}</div>
    """, small=True)
...
with c2:
    ...
    glass_card(f"""
      ...
      <div style="margin-top:0.5rem">{pill('Bullish' if macd_bull else 'Bearish', 'green' if macd_bull else 'red')}</div>
    """, small=True)

with c3:
    glass_card(f"""
      <div class="section-label" style="margin-bottom:0.6rem">Bollinger %B</div>
      <div class="stat-number" style="color:{GOLD}">
        {((last['Close']-last['BB_Lo'])/(last['BB_Up']-last['BB_Lo'])*100):.1f}%
      </div>
...
with c5:
    ...
    glass_card(f"""
      ...
      <div style="margin-top:0.5rem">{pill('MA50 > MA200' if cross else 'MA50 < MA200', 'green' if cross else 'red')}</div>
    """, small=True)
