...
Future price paths from real historical mu/sigma
"""
...
# ── Real data: mu and sigma from actual daily returns ─────────────────────────
...
with right:
    ...
    section_label("Historical Stats Used")
    glass_card(f"""
      {kv('Daily μ (mean)',   f"{mu*100:.4f}%",          OR2)}
      {kv('Daily σ (std)',    f"{sigma*100:.4f}%",        OR2)}
