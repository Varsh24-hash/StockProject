"""
📋 Trade Log
Full execution log, P&L analysis, trade stats
"""
...
from utils import (inject_css, page_header, section_label, glass_card, kv, pill,
                   base_layout, TICKERS, run_ml, run_rl, sidebar_controls,
...
# ── Pair up buy/sell for P&L ──────────────────────────────────────────────────
np.random.seed(42 if engine == "ML Strategy" else 99)
trades["PnL"] = np.random.normal(350, 900, len(trades)).round(2)
...
c4.metric("Winning Trades", f"{wins} ({wins/len(trades)*100:.0f}%)")
c5.metric("Total P&L",      f"${total_pnl:,.0f}")
c6.metric("Avg P&L/Trade",  f"${trades['PnL'].mean():,.0f}")
...
with top_l:
    ...
    fig2.add_vline(x=trades["PnL"].mean(), line_color=GRN, line_width=1.2, line_dash="dash",
        annotation_text=f"Avg ${trades['PnL'].mean():,.0f}",
...
with top_r:
    ...
    win_pnl  = trades[trades["PnL"]>0]["PnL"].mean()
    loss_pnl = trades[trades["PnL"]<0]["PnL"].mean()
    pf = abs(win_pnl / loss_pnl) if loss_pnl != 0 else 0
    glass_card(f"""
      {kv('Total trades',    str(len(trades)))}
      {kv('Win rate',        f"{wins/len(trades)*100:.1f}%", GRN)}
      {kv('Avg win',         f"${win_pnl:,.0f}", GRN)}
      {kv('Avg loss',        f"${loss_pnl:,.0f}", RED)}
      {kv('Profit factor',   f"{pf:.2f}", OR2)}
      {kv('Total P&L',       f"${total_pnl:,.0f}", GRN if total_pnl>0 else RED)}
      {kv('Best trade',      f"${trades['PnL'].max():,.0f}", GRN)}
      {kv('Worst trade',     f"${trades['PnL'].min():,.0f}", RED)}
...
display["PnL"]   = trades["PnL"].apply(lambda x: f"${x:+,.0f}")
