...
# ── Project root (3 levels up from trading_app/utils.py) ─────────────────────
...
def get_price_data(ticker: str) -> pd.DataFrame:
    """Load OHLCV from data/raw/<TICKER>.csv.
    The raw CSV has a double-header: row 0 = col names, row 1 = ticker label.
    """
...
@st.cache_data(show_spinner=False)
def get_processed(ticker: str) -> pd.DataFrame:
...
def get_features(ticker: str) -> pd.DataFrame:
    """OHLCV + full indicator set computed from real raw data."""
...
def run_ml(ticker: str, model: str, capital: float, txn: float):
    ...
    model_path = os.path.join(_MODEL_DIR, model_file_map.get(model, "xgb_model.pkl"))
...
def _run_ml_simulated(ticker, model, capital, txn):
    ...
    for i in range(len(df) - 1):
        ...
        if s == 1 and shares == 0:
            ...
            trades.append({"Date":df.index[i],"Type":"BUY","Price":round(p,2),"Shares":shares})
        elif s == 0 and shares > 0:
            cash += shares*p*(1-txn)
            trades.append({"Date":df.index[i],"Type":"SELL","Price":round(p,2),"Shares":shares})
            shares = 0
        port.append(cash + shares*p)
    df = df.iloc[:-1].copy()
    df["Portfolio"] = port
...
def calc_pnl(trades: pd.DataFrame, txn: float = 0.001) -> pd.DataFrame:
    """Pairs BUY/SELL round-trips and computes real net P&L per trade."""
    if trades.empty:
        return trades
    ...
    for i, row in trades.iterrows():
        if row["Type"] == "BUY":
            ...
            buy_idx, buy_price, buy_shares = buy_stack.pop(0)
...
def sidebar_controls():
    ...
    # ── Clear all caches and rerun when button is clicked ─────────────────
    if run:
